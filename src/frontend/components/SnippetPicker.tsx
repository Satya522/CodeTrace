...
export function SnippetPicker({ currentLanguage, onSelect }: SnippetPickerProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <BookOpen size={14} className="text-emerald-400 opacity-90" />
        Examples
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute left-0 lg:right-0 lg:left-auto top-full mt-2 w-64 rounded-2xl border border-white/10 bg-[#010409]/95 p-2 shadow-[0_12px_40px_rgba(0,0,0,0.8),0_0_30px_-10px_rgba(0,230,118,0.25)] backdrop-blur-2xl z-50 max-h-[70vh] overflow-y-auto custom-scrollbar"
          >
            {/* Header */}
            <div className="flex items-center gap-2 px-3 py-2 border-b border-white/10 mb-2">
              <Sparkles size={14} className="text-[#00E676]" />
              <span className="text-[11px] font-bold text-white/80 uppercase tracking-wider">Algorithm Examples</span>
            </div>

            {Object.entries(grouped).map(([category, items]) => (
              <div key={category} className="mb-2 last:mb-0">
                <div className="px-3 py-1 text-[10px] font-bold text-emerald-400/80 uppercase tracking-wider">
                ...
                <div className="flex flex-col gap-0.5">
                  {items.map((snippet) => (
                    <button
                      ...
                      className={`flex items-center justify-between w-full rounded-xl px-3 py-2 text-xs font-medium transition-colors duration-200 ${
                    ...
                    >
                      ...
                      {selectedId === snippet.id && <Check size={14} className="text-emerald-400" />}
                    </button>
                  ))}
                </div>
              </div>
